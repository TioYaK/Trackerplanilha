...
async function testPrintModalHTML() {
  ...
  try {
    ...
    console.log('7. Aguardando 3s para o modal renderizar...');
    await new Promise(r => setTimeout(r, 3000));

    const modalData = await page.evaluate(() => {
      const modal = document.querySelector('.turnstile-modal');
      const widget = document.querySelector('.turnstile-widget');
      return {
        modalInnerHTML: modal ? modal.innerHTML : null,
        widgetChildren: widget ? Array.from(widget.children).map(c => ({ tag: c.tagName, id: c.id, className: c.className, outerHTML: c.outerHTML })) : []
      };
    });
