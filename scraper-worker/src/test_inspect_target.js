...
async function testInspectTurnstileContainer() {
  ...
  try {
    ...
    const shadowCheck = await page.evaluate(() => {
      ...
      const targetDiv = widget.querySelector('div > div > div > div');
      if (!targetDiv) return { error: 'No targetDiv' };
      const shadow = targetDiv.shadowRoot;
      return {
        hasShadow: !!shadow,
        shadowInnerHTML: shadow ? shadow.innerHTML : null,
        targetDivHTML: targetDiv.outerHTML
      };
    });

    console.log('Resultado do targetDiv:', JSON.stringify(shadowCheck, null, 2));
