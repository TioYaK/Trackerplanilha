...
async function testSubmitInviteWithProfile() {
  const chromeExe = findChrome();
  const userDataDir = path.resolve('./worker_profiles/auroria_test');
  if (!fs.existsSync(userDataDir)) {
    fs.mkdirSync(userDataDir, { recursive: true });
  }

  const launchOpts = {
    headless: false, // Vamos testar non-headless ou headless: 'new'
    userDataDir,
    args: [
      '--no-sandbox',
  ...
  try {
    ...
    console.log('7. Aguardando 3s para o Turnstile modal aparecer...');
    await new Promise(r => setTimeout(r, 3000));

    // Tirar foto do modal no perfil persistente
    await page.screenshot({ path: 'C:/Users/YaKe/.gemini/antigravity/brain/4e6b1053-e550-48e6-b21f-3295a1f5ee45/scratch/profile_modal.png' });

    // Localizar widget box e clicar
    const widgetEl = await page.$('.turnstile-widget');
    if (widgetEl) {
      const box = await widgetEl.boundingBox();
      console.log('Widget BoundingBox:', box);
      if (box) {
        const clickX = Math.round(box.x + 25);
    ...
    const pageState = await page.evaluate(() => {
      ...
      return {
        ...
        successMsg: body.includes('Sucesso') || body.includes('convidado com sucesso') || body.includes('Xeroza pomba'),
        bodySnippet: body.substring(0, 1000)
      };
    });

    console.log('Estado final:', JSON.stringify(pageState, null, 2));
