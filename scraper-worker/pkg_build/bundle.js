...
var guildPage = null;
var highscoresPage = null;
var isRestarting = false;
var cfRestartCount = 0;
var MAX_CF_RESTARTS = 3;
var guildPageLock = Promise.resolve();
var highscoresPageLock = Promise.resolve();
async function isCloudflareBlocked(page) {
  try {
    const title = await page.title();
    const url2 = page.url();
    const html = await page.content();
    const titleBlocked = title.includes("Just a moment") || title.includes("Attention Required") || title.includes("Access denied") || title.includes("Please Wait");
    const urlBlocked = url2.includes("cloudflare") || url2.includes("/cdn-cgi/");
    const bodyBlocked = html.includes("cf-browser-verification") || html.includes("cf-challenge") || html.includes("cf_clearance") || html.includes("__cf_chl") || html.includes("Checking your browser") || html.includes("Enable JavaScript and cookies to continue");
...
async function closeBrowser() {
  try {
    if (guildPage) {
      await guildPage.close().catch(() => {
      });
      guildPage = null;
    }
    if (highscoresPage) {
      await highscoresPage.close().catch(() => {
      });
      highscoresPage = null;
    }
    if (globalBrowser) {
...
async function initBrowser() {
  ...
  globalBrowser = await import_puppeteer_extra.default.launch(launchOpts);
  ...
  console.log("[Scraper] \u{1F310} Browser iniciado.");
}
...
async function safeGoto(page, url2, { timeout = 3e4 } = {}) {
  try {
    await page.goto(url2, { waitUntil: "networkidle2", timeout });
  } catch (e) {
    console.warn(`[Scraper] Timeout/erro ao navegar para ${url2}: ${e.message}`);
  }
  if (await isCloudflareBlocked(page)) {
    console.log("[Scraper] Challenge do Cloudflare detectado. Aguardando resolu\xE7\xE3o...");
    try {
      await page.waitForNavigation({ waitUntil: "networkidle0", timeout: 2e4 });
    } catch {
    }
    if (await isCloudflareBlocked(page)) {
      console.warn("[Scraper] Cloudflare N\xC3O resolvido. Reiniciando browser...");
      return null;
    }
...
var CACHE_TTL = 15 * 1e3;
function readCache(name) {
  const p = import_path.default.join(CACHE_DIR, `cache_${name}.json`);
  if (!import_fs.default.existsSync(p)) return null;
  const age = Date.now() - import_fs.default.statSync(p).mtimeMs;
...
function writeCache(name, data) {
  ...
  try {
    import_fs.default.writeFileSync(import_path.default.join(CACHE_DIR, `cache_${name}.json`), JSON.stringify(data));
  } catch {
  }
}
async function scrapeGuild(guildName, maxPages = 50) {
  ...
  if (!page) return [];
  try {
    const url2 = `https://rubinot.com.br/guilds/${encodeURIComponent(guildName)}`;
    await safeGoto(page, url2);
    let pageNum = 1;
    let keepGoing = true;
    while (keepGoing && pageNum <= maxPages) {
      ...
      if (allMembers.length > 0 && allMembers[allMembers.length - 1].name === membersOnPage[membersOnPage.length - 1].name) {
        console.log(`[Scraper] Fim da pagina\xE7\xE3o detectado na p\xE1gina ${pageNum} (repetido).`);
        keepGoing = false;
        break;
      }
      allMembers.push(...membersOnPage);
      console.log(`[Scraper] Guilda ${guildName} - P\xE1gina ${pageNum}: ${membersOnPage.length} membros`);
      const hasNextButton = await page.evaluate(() => {
        const btns = Array.from(document.querySelectorAll("button"));
        const nextBtn = btns.find((b) => b.title === "Pr\xF3xima P\xE1gina" || b.innerHTML.includes("chevron-right"));
...
async function scrapeHighscores(world, onPageScraped, maxPages = 20) {
  ...
  try {
    ...
    const html = await safeGoto(page, "https://rubinot.com.br/highscores", { timeout: 3e4 });
    if (html === null) {
    ...
    if (world) {
      const optionValue = await page.evaluate((worldName) => {
        const opt = Array.from(document.querySelectorAll("select option")).find((o) => o.textContent.trim().toLowerCase() === worldName.toLowerCase());
    ...
    while (keepGoing && pageNum <= maxPages) {
      ...
      if (playersOnPage.length === 0) {
        ...
        console.log(`[Scraper] P\xE1gina ${pageNum}: ${playersOnPage.length} jogadores`);
...
// src/jobs/fetchGuild.js
var import_config = require("dotenv/config");
var runFetchGuild = async () => {
  ...
  try {
    ...
    const uniqueMembersMap = /* @__PURE__ */ new Map();
    members.forEach((m) => {
    ...
    try {
      ...
      let page = 0;
      while (true) {
        const { data: dbMembers } = await supabase.from("guild_members").select("name").range(page * 1e3, (page + 1) * 1e3 - 1);
...
var BASE_URL = process.env.AURORIA_BASE_URL || "https://auroriaglobal.com";
...
var fetchPage = async (path4) => {
  try {
    const response = await api.get(path4);
    return cheerio2.load(response.data);
  } catch (error) {
    console.error(`Erro ao fazer scraper na rota ${path4}:`, error.message);
    throw error;
  }
};

// src/jobs/fetchOnlines.js
var runFetchOnlines = async () => {
  const path4 = "/?subtopic=whoisonline";
  console.log(`[JOB] Fetching online players...`);
  try {
    const $ = await fetchPage(path4);
    const onlinePlayers = [];
    $("table tr").each((i, row) => {
      if (i === 0) return;
...
// src/jobs/fetchHighscores.js
var import_config2 = require("dotenv/config");
var runFetchHighscores = async () => {
  try {
    ...
    for (let i = 0; i < names.length; i += chunkSize) {
      ...
      const { data: lastLogs } = await supabase.from("telemetry_logs").select("character_name, xp_total").in("character_name", chunk).order("recorded_at", { ascending: false });
      if (lastLogs) {
    ...
    for (const player of players) {
      if (memberNames.has(player.name.toLowerCase())) {
        ...
        logsToInsert.push({
          ...
          is_online: false
          // Será atualizado por telemetry fetchGuild
        });
      }
    }
...
// src/jobs/auditSlots.js
var runAuditSlots = async () => {
  ...
  try {
    ...
    let page = 0;
    while (true) {
      const { data, error } = await supabase.from("parties_planilhadas").select("*").gte("created_at", twentyFourHoursAgo).range(page * 1e3, (page + 1) * 1e3 - 1);
    ...
    console.log(`[AUDIT] Hor\xE1rio atual (BRT - Bras\xEDlia): ${currentTimeStr}`);
    for (const party of parties) {
      ...
      if (!party.members || party.members.length === 0) {
        console.log(`[AUDIT] Party ${party.party_name} n\xE3o tem membros listados.`);
        continue;
      }
      const twentyMinsAgo = new Date(now.getTime() - 20 * 6e4).toISOString();
      const { data: logs, error: logsError } = await supabase.from("telemetry_logs").select("character_name, delta_xp, recorded_at").in("character_name", party.members).gte("recorded_at", twentyMinsAgo);
      let isHunting = false;
      let totalDelta = 0;
      ...
      if (isHunting) {
        ...
        if (currentInMinutes - slotStartInMinutes <= 20) {
          newStatus = "SUBOPTIMAL";
        }
      }
      const { error: updateError } = await supabase.from("parties_planilhadas").update({
        status: newStatus,
        // Formatando o DeltaXP para M ou K para ficar amigável no Dashboard
        delta_xp: totalDelta > 1e6 ? (totalDelta / 1e6).toFixed(1) + "M" : (totalDelta / 1e3).toFixed(0) + "K"
      }).eq("id", party.id);
      if (updateError) {
      ...
      if (newStatus === "GHOST_SLOT") {
        ...
        const { data: existingStrikes } = await supabase.from("player_strikes").select("id").eq("reason", strikeReason).limit(1);
        if (!existingStrikes || existingStrikes.length === 0) {
          console.log(`[AUDIT] \u{1F6A8} Aplicando Strikes Autom\xE1ticos para GHOST_SLOT: ${party.party_name}`);
          const expiresAt = /* @__PURE__ */ new Date();
          expiresAt.setDate(expiresAt.getDate() + 3);
          const newStrikes = party.members.map((member) => ({
            character_name: member,
            reason: strikeReason,
            admin_name: "Rob\xF4 Xerife",
    ...
    console.log(`[AUDIT] Auditoria conclu\xEDda com sucesso.`);
  } catch (error) {
    console.error(`[AUDIT] Erro cr\xEDtico na auditoria:`, error.message);
  }
};

// src/jobs/syncBankTS3.js
var import_net = __toESM(require("net"), 1);
function parseTS3String(str) {
  if (!str) return "";
  return str.split("\\s").join(" ").split("\\p").join("|").split("\\/").join("/").split("\\c").join(":");
}
function extractName(desc) {
  if (!desc) return "Sem Descri\xE7\xE3o";
  const cleanDesc = parseTS3String(desc);
  const match = cleanDesc.match(/Main:\s*([^|]+)/i);
  if (match) return match[1].trim();
  return "N\xE3o Identificado";
}
var TS3Client = class {
  constructor() {
...
var runBankSync = async () => {
  console.log("[TS3_SYNC] Iniciando conex\xE3o segura com ClientQuery (localhost:25639)...");
  ...
  try {
    ...
    if (!authRes || !authRes.includes("error id=0 msg=ok")) {
      console.error("[TS3_SYNC] Falha na autentica\xE7\xE3o da API KEY.");
      ts3.close();
      return;
    }
    ...
    const testResults = [];
    console.log(`[TS3_SYNC] Encontrados ${rawClients.length} usu\xE1rios. Analisando descri\xE7\xF5es...`);
    for (const rc of rawClients) {
      if (!rc.includes("clid=")) continue;
      if (rc.includes("client_type=1")) continue;
      ...
      testResults.push({
        ...
        Tem_FBot_Bank: groups.includes("292") ? "SIM" : "N\xC3O",
        Descricao_Original: parseTS3String(desc).substring(0, 40) + "..."
      });
    }
    ts3.close();
    const paidUsers = testResults.filter((u) => u.Tem_FBot_Bank === "SIM" && u.Char_Extraido !== "N\xE3o Identificado" && u.Char_Extraido !== "Sem Descri\xE7\xE3o");
    console.log(`
[TS3_SYNC] Sincronizando ${paidUsers.length} pagantes com o Banco de Dados...`);
    const currentMonth = (/* @__PURE__ */ new Date()).toISOString().slice(0, 7);
    await supabase.from("guild_bank_payments").delete().eq("payment_month", currentMonth).eq("admin_name", "TS3_Sync");
    if (paidUsers.length > 0) {
      const inserts = paidUsers.map((u) => ({
        character_name: u.Char_Extraido,
        payment_month: currentMonth,
        amount: 250,
        // O user disse que é 250rc
        admin_name: "TS3_Sync"
      }));
      ...
      console.log("[TS3_SYNC] Ningu\xE9m encontrado com a tag.");
    }
  } catch (err) {
    console.error("[TS3_SYNC] Erro Cr\xEDtico:", err.stack);
    ts3.close();
  }
};
...
var checkForUpdates = async () => {
  if (!process.pkg) {
    return new Promise((resolve) => {
      (0, import_child_process.exec)("git pull", (error, stdout) => {
        if (stdout && !stdout.includes("Already up to date")) {
          console.log("[UPDATER] Nova atualiza\xE7\xE3o do Git! Reiniciando...");
          process.exit(0);
        } else {
          resolve(false);
        }
  ...
  try {
    console.log("[UPDATER] Checando novas vers\xF5es no Supabase...");
    const { data: versionData, error: versionErr } = await supabase.storage.from("releases").download("version.json");
    if (versionErr || !versionData) return false;
    const text = await versionData.text();
    const remoteVersion = JSON.parse(text).version;
    if (remoteVersion > CURRENT_VERSION) {
      console.log(`[UPDATER] Vers\xE3o ${remoteVersion} encontrada! Baixando atualiza\xE7\xE3o silenciosa...`);
      const { data: exeData, error: exeErr } = await supabase.storage.from("releases").download("auroria-worker.exe");
      if (exeErr || !exeData) {
        console.error("[UPDATER] Erro ao baixar o EXE:", exeErr);
        return false;
      }
      const buffer = Buffer.from(await exeData.arrayBuffer());
      const newExePath = import_path3.default.join(process.cwd(), "auroria-worker_new.exe");
      const currentExePath = process.execPath;
      const batPath = import_path3.default.join(process.cwd(), "update.bat");
      import_fs2.default.writeFileSync(newExePath, buffer);
      const batCode = `
@echo off
...
del "%~f0"
`;
      import_fs2.default.writeFileSync(batPath, batCode);
      console.log("[UPDATER] Atualiza\xE7\xE3o baixada com sucesso! Aplicando e reiniciando...");
...
// src/index.js
import_dotenv3.default.config({ path: import_path4.default.join(process.cwd(), ".env") });
var WORKER_ID = `worker-${Math.random().toString(36).substring(2, 9)}`;
var POLL_INTERVAL = 5e3;
var LOCK_TIMEOUT_MINUTES = 3;
console.log(`[WORKER] Iniciando worker ID: ${WORKER_ID}`);
var fetchTask = async () => {
  try {
    const timeLimit = /* @__PURE__ */ new Date();
    timeLimit.setMinutes(timeLimit.getMinutes() - LOCK_TIMEOUT_MINUTES);
    const { data: tasks, error } = await supabase.from("task_queue").select("*").or(`status.eq.PENDING,and(status.eq.IN_PROGRESS,locked_at.lt.${timeLimit.toISOString()})`).order("updated_at", { ascending: true }).limit(1);
    if (error) throw error;
    if (!tasks || tasks.length === 0) return null;
    const task = tasks[0];
    const { data: updatedTask, error: updateError } = await supabase.from("task_queue").update({
      status: "IN_PROGRESS",
      worker_id: WORKER_ID,
      locked_at: (/* @__PURE__ */ new Date()).toISOString()
    }).eq("id", task.id).eq("status", task.status).select().single();
...
var completeTask = async (task) => {
  if (task.task_type === "SYNC_BANK_TS3") {
    ...
    await supabase.from("task_queue").update({ status: "PENDING", locked_at: null, worker_id: null }).eq("id", task.id);
  }
};
var requeueTask = async (task) => {
...
loop();
setInterval(async () => {
  console.log("[CRON] Rodando Sincroniza\xE7\xE3o Autom\xE1tica do TS3 (30 min)...");
  await runBankSync();
}, 30 * 60 * 1e3);
